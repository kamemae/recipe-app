...
export default function InspirationGenerator({ recipes }) {
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md mb-8">
      <h2 className="text-2xl font-semibold mb-4">Generator Inspiracji</h2>
      <input value={query} onChange={e => setQuery(e.target.value)} placeholder="Podaj składniki (np. jajka, mleko)" className="input" />
      <button onClick={searchRecipes} className="btn mt-2">Szukaj przepisów</button>
      {results.length > 0 && (
        <div className="mt-4">
          <h3 className="text-xl font-semibold mb-2">Pasujące przepisy:</h3>
